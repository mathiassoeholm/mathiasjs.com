...
Our application is divided into an authenticated version and an unauthenticated version. We will start by creating the component tree for the unauthenticated app, which involves
the signup and login screens.

Our routing requirements are very small, that's why we'll use a minimalist router called "wouter". It's similar to the declarative routing used in React Router.
It even provides the familiar Route, Link, Switch and Redirect components.

```shell
npm i wouter
```

We can now set up the routes in `src/UnauthenticatedApp.js`:
...
The default route `/` will now redirect to `/login` which will render the Login component. Going to `/signup` will show the Signup component.
All other routes will simply show the "404, not found!" message.
