---
title: How to build an authentication service with Netlify Functions
date: 2020-01-27
---
...
What we will have though is a secure email and password based _stateless_ authentication service.
...
We want to achieve a way of verifying that a user is who they say they are. Furthermore we want to do this without querying a database. Having the user send an email/password combination on every request will not work, because we will need to check the database to see if the password is correct.

We also can't keep track of a Session ID because it will require a lookup in a datastore when using a serverless function, which is what we're trying to avoid.

I want to explain how token-based authentication works, with a little story:

---

**Alice and the server**

You are a server, you can make beep noises if you wish.
...
You find a piece of paper and write alice@gmail.com on it. You decide that Alice is allowed to upload media, cast votes and chat so you also write that on the paper.
...
Several months later, Alice wants to cast a vote in a poll that Bob made. But Alice lost the paper you gave her.
...
We will bootstrap our React application with [create-react-app](https://create-react-app.dev/). It lets you set up a React project with a single command.
...
But make sure you DON'T create a README and DON'T select .gitignore or license. GitHub even tells us to skip this step if we're importing an existing repository:
...
Netlify is now building your app, and a link should show when it's done. In fact, it will now build and publish the app every time you push to the master branch in your Git repository. Simply amazing! Thanks Netlify for making this so easy 🙌

### Set up Netlify Functions workflow

Note that I use the term serverless functions and Netlify Functions interchangeably. You can say that Netlify Functions are a specific vendor of serverless functions.

We want to transpile our Netlify Functions, so we can use ES6 features, so we'll need an extra bit of configuration, but it's not that bad. Luckily we don't have to write any Webpack configs.
...
This tells Netlify where to look for our serverless functions. We then install two dev dependencies, which we will use to transpile our functions.
...
We need to adjust the current build script in our package.json file, so we build both the client code and the serverless. Update your package.json file, so you have the following scripts:
...
Another thing that you will want to do is add `/built-lambda` to the .gitignore.
...
So we don't care what port the serverless function is running on and it saves us from some extra configuration.
...
Then check your database on MongoDG Atlas afterwards.
...
The way you clear a cookie is either by setting the expiry date before the current date.
...
but all request to protected endpoints will result in 401.
